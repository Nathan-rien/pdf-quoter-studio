...
import { DEFAULT_CONTRACT_PAGES, OPTIONS_PER_PAGE, LINES_PER_PAGE, CANVAS_SCALE, CANVAS_DISPLAY_MAX_WIDTH, INVEST_LINES_PAGE1, INVEST_LINES_CONTINUATION, computeFooterLines, INVEST_SINGLE_PAGE_FOOTER_THRESHOLD, SERVICES_ITEMS_PAGE1, SERVICES_ITEMS_CONTINUATION, estimateVisualLines } from '@/lib/canvas-constants';
...
export function RentalProposalExport() {
  ...
  const generateDynamicContentByPage = useCallback((): { content: Record<number, string>; excludeIds: Record<number, string[]>; extraPagesAfter: Record<number, string[]> } => {
    ...
    const makeRowHTML = (ligne: typeof lignesData[0]) => {
      ...
      return `
      <tr>
        <td style="padding: ${cellPadding}; border-bottom: 1px solid #e5e7eb; word-wrap: break-word; white-space: pre-wrap; max-width: 60%;">${ligne.designation || '-'}</td>
