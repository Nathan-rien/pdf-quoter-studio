...
export async function generateServiceProposalHtml(
  ...
  const renderServiceZone = (zone: PositionedDynamicZone) => {
    ...
    if (zone.type === 'service_external_providers') {
      // Prestataires extérieurs: jamais affiché dans les PDF (devis ou contrat),
      // uniquement dans les champs UI du contrat services.
      return '';
    }
  ...
  const renderCgHeader = (title: string, hPad: string = '10mm') => `
    <div style="position:relative;background:#000000;color:#ffffff;font-family:'Outfit',sans-serif;font-weight:700;letter-spacing:0.4px;text-transform:uppercase;height:17mm;padding:0 220px 0 ${hPad};display:flex;align-items:center;box-sizing:border-box;overflow:hidden;">
      ...
      <img src="${cbproWhiteLogo}" alt="Cybertek Pro" style="position:absolute;right:8px;top:50%;transform:translateY(-50%);width:210px;height:60px;object-fit:contain;object-position:right center;" />
    </div>
  `;
  ...
  const renderArticle = (el: any): { html: string; chars: number; isTitle: boolean } => {
    ...
    if (isTitle) {
      ...
      const marginTop = isXIII ? '10mm' : '4mm';
      return {
        html: `<h3 style="font-family:'Outfit',sans-serif;font-size:10.5px;font-weight:700;color:#111111;text-transform:uppercase;letter-spacing:0.05em;margin:${marginTop} 0 2mm 0;padding-bottom:1mm;border-bottom:1px solid #e5e7eb;break-after:avoid;break-inside:avoid;-webkit-column-break-after:avoid;-webkit-column-break-inside:avoid;page-break-inside:avoid;">${escCg(raw)}</h3>`,
        chars: raw.length,
        isTitle: true,
      };
    }
